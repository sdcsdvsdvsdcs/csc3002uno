...
public:
    ...
    void setRole(std::unique_ptr<Role> r) {
        role = std::move(r);
    }
    ...
    bool hasCardType(bool actionCards) const {
        // 简化实现，总是返回true
        return true;
    }
    
    Card stealRandomCardOfType(bool actionCards) {
        // 简化实现
        if (hand.empty()) return Card();
        Card card = hand.back();
        hand.pop_back();
    ...
    void setCurrentSkill(const std::string& skill) {
        currentSkill = skill;
    }
...
public:
    Card(const std::string& c = "", const std::string& v = "", bool action = false) 
        : color(c), value(v), isActionCard(action) {}
    
    std::string toString() const {
        return color + " " + value;
    }
    
    bool getIsActionCard() const {
        return isActionCard;
    }
};
...
public:
    std::vector<Card> peekTopCards(int count) {
        // 简化实现
        return {Card("红", "5"), Card("蓝", "跳过", true), Card("绿", "8")};
    }
    ...
    std::vector<Card> getDiscardPile() {
        // 简化实现
        return {Card("黄", "反转", true), Card("红", "2"), Card("蓝", "5")};
    }
    
    bool isJustDiscarded(const Card& card) {
        // 简化实现
        return false;
    }
    
    void removeCardFromDiscard(int index) {
        // 简化实现
    }
    
    void revealRole(Player& player) {
        std::cout << player.getName() << "的角色是：" << player.getRole()->getName() << std::endl;
    }
};
...
int main() {
    ...
    player1.setRole(std::make_unique<LuckyStar>());
    player2.setRole(std::make_unique<Collector>());
    player3.setRole(std::make_unique<Thief>());
    player4.setRole(std::make_unique<Defender>());
    
    // 演示幸运星技能
    std::cout << "=== 幸运星技能演示 ===" << std::endl;
    std::cout << player1.getRole()->getInfo() << std::endl;
    // player1.getRole()->useSkill(player1, game); // 取消注释可以测试交互
    
    // 演示收藏家技能
    std::cout << "\n=== 收藏家技能演示 ===" << std::endl;
    std::cout << player2.getRole()->getInfo() << std::endl;
    
    // 演示小偷技能
    std::cout << "\n=== 小偷技能演示 ===" << std::endl;
    std::cout << player3.getRole()->getInfo() << std::endl;
    // player3.getRole()->useSkill(player3, game, &player1); // 取消注释可以测试交互
    
    // 演示防御者技能
    std::cout << "\n=== 防御者技能演示 ===" << std::endl;
    std::cout << player4.getRole()->getInfo() << std::endl;
    
    // 演示防御者被动防御
    std::cout << "\n=== 防御者被动防御演示 ===" << std::endl;
    Defender* defender = dynamic_cast<Defender*>(player4.getRole());
    if (defender) {
        player3.setCurrentSkill("thief_skill");
        bool defended = defender->tryDefend(player3, player4, game);
        if (defended) {
