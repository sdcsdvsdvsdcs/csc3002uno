...
struct GameConfigInfo {
    ...
    // 新增：角色系统配置
    bool mEnableCharacters{true};
    Common::GameMode mGameMode{Common::GameMode::WITH_CHARACTERS};
};
...
struct CommonConfigInfo {
    ...
    std::optional<std::string> mWildEscape;
    
    // 新增：角色系统配置
    std::optional<bool> mEnableCharacters;
    std::optional<int> mMaxSkillUses;
    std::optional<std::string> mGameMode;
};
...
private:
    ...
    // 新增：游戏模式选项
    const static std::string CMD_OPT_SHORT_MODE;
    const static std::string CMD_OPT_LONG_MODE;
    const static std::string CMD_OPT_BOTH_MODE;
    const static std::string CMD_OPT_LONG_NO_CHARACTERS;
    ...
    const static std::string FILE_OPT_WILD;
    
    // 新增：配置文件选项
    const static std::string FILE_OPT_GAME_MODE;
    const static std::string FILE_OPT_ENABLE_CHARACTERS;
    const static std::string FILE_OPT_MAX_SKILL_USES;
};
}}
